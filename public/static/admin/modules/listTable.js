layui.define(['utable', 'uform', 'request', 'laypage', 'layer', 'custormEvent'], function (exports) {
  ...
  function listTableRender(url, cols, config, extendFun, tableNameId, doneFun,closeHandelBtnFun) {
    ...
    var default_config = {
      ...
      even: false, //开启隔行背景
      limits: [
        10, 15, 20, 30, 40, 50, 70, 100, 120, 150, 200
      ],
      text: {
    ...
    if(listConfig.tableConfig){
      render_config = $.extend(render_config, listConfig.tableConfig);
    }
  ...
  //顶部操作修改字段
  function doBatchHandle(callFun) {
    var tableNameId = $(this).data('tableNameId') || 'LAY-list-table';
    var ids = listGetCheckedBoxId(tableNameId);
    if (ids.length <= 0) {
      layer.msg(appLang.trans('没有选择数据'));
    ...
    if(inputType==='checkbox'){
       var inputObj = $('input[name^="'+field+'["]:checked');
       value = listGetCheckedBoxIdByData(inputObj);
    }
    console.log(value);
    var data = {
      field: field,
  ...
  function listGetCheckedBoxIdByData(checkData) {
  ...
  function listSearch(filterName, tableNameId) {
    ...
    form.on('submit(' + filterName + ')', function (data) {
      ...
      reloadConfig.where = field;
      reloadConfig.where._token = $('[name="csrf-token"]').attr('content');
      // console.log(reloadConfig.where);
      table.render(reloadConfig);
    });
  }
