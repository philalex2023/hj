 ...
 */
class AXController extends PayBaseController implements Pay
{
    use PayTrait;
    use ApiParamsTrait;
    use IpTrait;

    public string $payFlag = 'AX';
    ...
    function sign($data, $md5Key): string
    {
        ...
        Log::info('ax_sign_origin:',[$md5str]);
        $res = md5($md5str . $md5Key);
        Log::info('ax_sign_res:',[$res]);
        return $res;
    }
    ...
    function verify($data, $md5Key, $pubKey): bool
    {
        ...
        Log::info('ax_callback_sign_origin',$returnArray);
        $sign = md5(implode('', $returnArray) . $md5Key);
        Log::info('ax_callback_sign_res',[$sign]);
        if ($sign == $pubKey) {
            return true;
        }
        return false;
