 ...
 */
class HYController extends PayBaseController implements Pay
{
    use PayTrait;
    use ApiParamsTrait;
    use IpTrait;
    public string $flag = 'HY';
    /**
     * 长江支付动作
    ...
    public function pay(Request $request): JsonResponse
    {
        ...
        Log::info($this->flag.'_pay_params===', [$params]);//参数日志
        // 强制转换
        try {
            $payEnv = self::getPayEnv();
            $secret = json_decode($payEnv[$this->flag]['secret'],true);
            ...
            $mercId = $payEnv[$this->flag]['merchant_id'];
            $notifyUrl = 'https://' .$_SERVER['HTTP_HOST'] . $payEnv[$this->flag]['notify_url'];
            $oldMix = false;
            ...
            $curl = (new Client([
                ...
                'verify' => false,
            ]))->post($payEnv[$this->flag]['pay_url'], ['form_params' => $input]);
    ...
    public function callback(Request $request): mixed
    {
        ...
        try {
            $payEnv = self::getPayEnv()[$this->flag];
        ...
        Log::info($this->flag.'_pay_callback_res===', [$return]);
        return response($return);
    }

    public function sign($data, $md5Key, $privateKey): string
    {
        ksort($data);
        reset($data);
        $arg = '';
    ...
    public function verify($data,$md5Key,$pubKey){
        //验签
        ksort($data);
        reset($data);
        $arg = '';
